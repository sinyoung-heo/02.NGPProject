...
void CPotalDungeon::LateInit()
{

}

int CPotalDungeon::Update()
{
	CObj::LateInit();
	if (m_bIsDead)
		return DEAD_OBJ;
...
void CPotalDungeon::LateUpdate()
{
	CObj::MoveFrame();
}

void CPotalDungeon::Render(HDC hDC)
{
	CObj::UpdateRect();

	HDC hMemDC = nullptr;
	NULL_CHECK(hMemDC = CBmpMgr::GetInstance()->FindImage(L"Portal"));
